...
- Tiles that are not part of a preset keep working as before, and are always shown for a wave / role without an active
  preset. Whether they are also shown alongside an active preset is toggleable, in the editor, in the plugin's config
  ("Show non-preset tiles with a preset"), and in the sidebar panel.
